...
def requires_root(template: str):
    ...
    def decorator(func):
        ...
        func.__root_template__ = template
        return func
    return decorator
...
class RootsMiddleware(WagsMiddlewareBase):
    ...
    async def on_notification(
        self,
        context: MiddlewareContext[Notification],
        call_next: CallNext[Notification, Any]
    ) -> Any:
    ...
    async def handle_on_tool_call(
        ...
        try:
            ...
            param = str(e).strip("'")
            raise ValueError(
                f"Missing required parameter '{param}' for root validation"
            )
        ...
        raise PermissionError(
            f"Access denied: {resource} not in allowed roots"
        )
