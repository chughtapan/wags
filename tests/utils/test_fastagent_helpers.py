...
class TestMessageSerializer:
    ...
    def test_serialize_simple_message(self) -> None:
        ...
        msg = PromptMessageExtended(role="user", content=[TextContent(type="text", text="Hello, world!")])
    ...
    def test_preserves_tool_calls(self) -> None:
        ...
        tool_call = CallToolRequest(
            method="tools/call",
            params=CallToolRequestParams(name="test_tool", arguments={"arg1": "value1", "arg2": 42}),
        )
    ...
    def test_extract_tool_calls_by_turn(self) -> None:
        """Test extraction of tool calls grouped by turn."""
        # Create a complete conversation
        complete_data = {
            "messages": [
                {"role": "user", "tool_calls": None},
                {"role": "assistant", "tool_calls": {"t1": {"name": "search", "arguments": {"query": "test"}}}},
                {"role": "user", "tool_calls": None},  # New turn
                {"role": "assistant", "tool_calls": {"t2": {"name": "calculate", "arguments": {"x": 5}}}},
            ]
        }
    ...
    def test_format_to_executable(self) -> None:
