...
class TestHandlers:
    ...
    async def create_issue(self, owner: str, repo: str, title: str) -> dict:
    ...
    async def call_api(self, endpoint: str, data: str) -> dict:
        """Call an API endpoint."""
        return {"called": endpoint, "data": data}

    async def unprotected_method(self, data: str) -> dict:
...
class TestRootsMiddlewareIntegration:
    ...
    async def test_roots_validation_allows_matching_literal(self):
        ...
        async with Client(mcp, roots=["https://github.com/myorg/"]) as client:
            ...
            result = await client.call_tool(
                "create_issue",
                {"owner": "myorg", "repo": "test-repo", "title": "Test Issue"}
            )
            ...
            with pytest.raises(Exception) as exc_info:
                await client.call_tool(
                    "create_issue",
                    {"owner": "other-org", "repo": "test", "title": "Test"}
                )
            assert "Access denied" in str(exc_info.value)

    async def test_roots_validation_with_org_prefix(self):
        ...
        async with Client(mcp, roots=["https://github.com/myorg/"]) as client:
            ...
            result = await client.call_tool(
                "create_issue",
                {"owner": "myorg", "repo": "repo1", "title": "Test"}
            )
            assert "myorg/repo1" in result.data["created"]

            result = await client.call_tool(
                "create_issue",
                {"owner": "myorg", "repo": "repo2", "title": "Test"}
            )
            ...
            with pytest.raises(Exception) as exc_info:
                await client.call_tool(
                    "create_issue",
                    {"owner": "other-org", "repo": "repo1", "title": "Test"}
                )
            assert "Access denied" in str(exc_info.value)

    async def test_roots_change_notification_updates_validation(self):
        ...
        async with Client(mcp, roots=dynamic_roots_handler) as client:
            ...
            result = await client.call_tool(
                "create_issue",
                {"owner": "org1", "repo": "repo", "title": "Test"}
            )
            ...
            with pytest.raises(Exception):
                await client.call_tool(
                    "create_issue",
                    {"owner": "org2", "repo": "repo", "title": "Test"}
                )
    ...
    async def test_no_roots_fails_closed(self):
        ...
        async with Client(mcp) as client:
            ...
            with pytest.raises(Exception) as exc_info:
                await client.call_tool(
                    "create_issue",
                    {"owner": "any", "repo": "repo", "title": "Test"}
                )
            assert "No roots configured" in str(exc_info.value)

    async def test_unprotected_methods_bypass_validation(self):
        ...
        @mcp.tool
        async def unprotected_method(data: str) -> dict:
    ...
    async def test_multiple_roots_any_match_allows(self):
        ...
        async with Client(mcp, roots=roots) as client:
            ...
            result = await client.call_tool(
                "create_issue",
                {"owner": "allowed-org", "repo": "test", "title": "Test"}
            )
            ...
            with pytest.raises(Exception):
                await client.call_tool(
                    "call_api",
                    {"endpoint": "v3/users", "data": "test"}
                )

    async def test_specific_repo_root(self):
        ...
        async with Client(mcp, roots=["https://github.com/myorg/specific-repo"]) as client:
            ...
            result = await client.call_tool(
                "create_issue",
                {"owner": "myorg", "repo": "specific-repo", "title": "Test"}
            )
            ...
            with pytest.raises(Exception):
                await client.call_tool(
                    "create_issue",
                    {"owner": "myorg", "repo": "other-repo", "title": "Test"}
                )

    async def test_multiple_org_prefix_roots(self):
        ...
        async with Client(mcp, roots=roots) as client:
            ...
            result = await client.call_tool(
                "create_issue",
                {"owner": "public", "repo": "any-repo", "title": "Test"}
            )
            ...
            result = await client.call_tool(
                "create_issue",
                {"owner": "partner", "repo": "specific", "title": "Test"}
            )
            ...
            with pytest.raises(Exception):
                await client.call_tool(
                    "create_issue",
                    {"owner": "private", "repo": "repo", "title": "Test"}
                )