...
class TestSubstituteEnvVars:
    ...
    def test_substitute_env_vars(self) -> None:
        ...
        with patch.dict(os.environ, {"TEST_VAR": "test_value", "API_KEY": "secret"}):
            env_dict = {"VAR1": "${TEST_VAR}", "KEY": "${API_KEY}"}
            result = _substitute_env_vars(env_dict)
            assert result == {"VAR1": "test_value", "KEY": "secret"}
    ...
    def test_no_substitution_needed(self) -> None:
        ...
        env_dict: dict[str, Any] = {"STATIC": "value", "NUMBER": 42}
        result = _substitute_env_vars(env_dict)
        assert result == env_dict

    def test_mixed_env_values(self) -> None:
        """Test env dict with both template and regular values."""
        with patch.dict(os.environ, {"TOKEN": "abc123"}):
            env_dict: dict[str, Any] = {"TOKEN": "${TOKEN}", "STATIC": "fixed_value", "NUMBER": 42}
            result = _substitute_env_vars(env_dict)
            assert result == {"TOKEN": "abc123", "STATIC": "fixed_value", "NUMBER": 42}
    ...
    def test_malformed_placeholder(self) -> None:
        """Test that malformed placeholders are left unchanged."""
        env_dict = {"BAD1": "${INCOMPLETE", "BAD2": "MISSING_BRACE}", "GOOD": "normal_value"}
        result = _substitute_env_vars(env_dict)
...
class TestLoadConfig:
    ...
    def test_load_valid_mcp_config(self) -> None:
        """Test loading a valid MCP config file."""
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            config_data: dict[str, Any] = {"mcpServers": {"test": {"transport": "stdio", "command": "test"}}}
    ...
    def test_load_no_mcpservers_raises_error(self) -> None:
        """Test loading config without mcpServers raises error."""
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            config_data: dict[str, Any] = {"key": "value", "number": 42}
            json.dump(config_data, f)
            temp_path = f.name
    ...
    def test_load_multiple_servers_raises_error(self) -> None:
    ...
    def test_load_empty_mcpservers_raises_error(self) -> None:
    ...
    def test_load_with_env_substitution(self) -> None:
        ...
        with patch.dict(os.environ, {"TEST_VAR": "substituted"}):
            with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
                config_data: dict[str, Any] = {"mcpServers": {"test": {"env": {"KEY": "${TEST_VAR}"}}}}
                json.dump(config_data, f)
                temp_path = f.name
    ...
    def test_load_invalid_env_section(self) -> None:
